...
static double init_key = 0;// static_cast<double>(caffe_rng_rand());
...
// Usage: caffe_('solver_update_itersize', hSolver, iter_size)
static void solver_update_itersize(MEX_ARGS) {
  mxCHECK(nrhs == 2 && mxIsStruct(prhs[0]) && mxIsDouble(prhs[1]),
      "Usage: caffe_('solver_step', hSolver, iters)");
  Solver<float>* solver = handle_to_ptr<Solver<float> >(prhs[0]);
  int iter_size = mxGetScalar(prhs[1]);
  solver->Update_for_itersize(iter_size);
}
...
// Usage: caffe_('solver_clear_param_diff', solver)
static void solver_clear_param_diff(MEX_ARGS) {
  mxCHECK(nrhs == 1 && mxIsStruct(prhs[0]),
    "Usage: caffe_('solver_clear_param_diff', hSolver)");
  Solver<float>* solver = handle_to_ptr<Solver<float> >(prhs[0]);
  solver->ClearParamDiffs();
}
...
static handler_registry handlers[] = {
  ...
  { "solver_update_itersize",        solver_update_itersize         },
  { "get_net",                       get_net                        },
  { "net_get_attr",                  net_get_attr                   },
  { "solver_clear_param_diff",       solver_clear_param_diff        },
